...
# Upper bound on tracked_invites entries kept in memory / shown on the panel.
# Discord embeds allow 25 fields total (24 invite slots + the Summary field),
# and the 5-minute check task walks every tracked code — both want this set
# bounded. Oldest EXPIRED entries are pruned first (see _prune_tracked).
_TRACKED_INVITE_CAP: int = 24
...
class InviteManager:
    ...
    async def create_invite(self, guild: discord.Guild, max_uses: int, max_age: int = 0) -> Optional[discord.Invite]:
        for channel in guild.text_channels:
            if not channel.permissions_for(guild.me).create_instant_invite:
                continue
            try:
                invite = await channel.create_invite(max_uses=max_uses, max_age=max_age, unique=True, reason="Auto-generated by the invite manager")
                logging.info(f"[InviteManager] Created invite {invite.code} with {max_uses} max uses")
                return invite
            except discord.Forbidden:
                # Permission revoked between the local check and the API call
                # (or a channel-level overwrite) — fall through to the next
                # channel instead of killing the whole regeneration.
                continue
            except discord.HTTPException as e:
                # e.g. the guild reached Discord's active-invite cap. Trying
                # other channels rarely helps for cap errors, but the loop is
                # cheap and the per-channel log makes the cause visible.
                logging.warning(f"[InviteManager] Could not create invite in #{channel.name}: {e}")
                continue
        return None
    
    async def check_invite_status(self, guild: discord.Guild, invite_code: str, invites: Optional[List[discord.Invite]] = None) -> Dict[str, Any]:
        try:
            if invites is None:
    ...
    async def update_invite_message(self, channel: discord.TextChannel) -> bool:
        ...
        try:
            ...
            # The bot lost Read Message History in the invite channel. Say so
            # and bail out cleanly — previously this bubbled up as an
            # unhandled error and froze whatever progress message the caller
            # (e.g. !regenerateinvites) was driving.
            logging.warning("[InviteManager] Cannot update the tracking message: missing Read Message History in the invite channel")
            return False
    ...
    async def check_and_update_all(self, guild: discord.Guild, channel: discord.TextChannel) -> bool:
        any_changed = False
        # PERF/robustness: ONE invite-list fetch for the whole cycle. This used
        # to call check_invite_status per code — each fetching the ENTIRE guild
        # invite list — so a 24-code batch meant 24 full fetches every check
        # interval. If the fetch itself fails we skip the cycle instead of
        # marking every invite expired (a transient 403 used to do exactly
        # that, then the auto-regen "revived" dead batches).
        try:
            live_invites = await guild.invites()
        except Exception as e:
            logging.warning(f"[InviteManager] Skipping invite check cycle (could not fetch guild invites): {e}")
            return False
        for code in list(self.tracked_invites.keys()):
            status = await self.check_invite_status(guild, code, live_invites)
    ...
    def _prune_tracked(self) -> int:
        """Drop the oldest EXPIRED entries beyond _TRACKED_INVITE_CAP.

        The auto-regen merge (below) used to append every new batch to
        tracked_invites without ever removing the dead ones, so the set grew
        by 3 per cycle forever — the user's live DB had already reached 24
        entries (8 batches). Beyond bloating the panel embed (25-field
        Discord limit) it multiplied the check task's work every cycle.
        Insertion order = batch age, so pruning from the front of the expired
        entries drops the oldest history first. Active invites are never
        dropped. Returns the number of entries pruned.
        """
        if len(self.tracked_invites) <= _TRACKED_INVITE_CAP:
            return 0
        overflow = len(self.tracked_invites) - _TRACKED_INVITE_CAP
        dropped = 0
        for code in list(self.tracked_invites.keys()):
            if dropped >= overflow:
                break
            data = self.tracked_invites[code]
            if data.get('expired', False) or data.get('status') == 'expired':
                del self.tracked_invites[code]
                dropped += 1
        return dropped
    ...
    async def regenerate_expired(self, guild: discord.Guild, channel: discord.TextChannel, archived_by: Optional[int] = None) -> bool:
        ...
        if all_expired:
            ...
            # can show it later. Archived AFTER minting is confirmed: if the
            # mint fails we return early, so a permanently broken mint no
            # longer re-archives the same dead batch every check cycle.
            # Non-fatal if the archive itself fails.
            try:
            ...
            # ROOT-CAUSE FIX: the replaced batch used to stay live in Discord
            # (only the in-memory/DB tracking moved on), so every auto-regen
            # cycle left another bot-created invite behind. Over weeks that
            # accumulation piled up hundreds of live bot invites — which then
            # made a manual !regenerateinvites grind through all of them with
            # no visible progress ("stuck on Regenerating Invites"). Delete
            # the replaced invites from Discord too — best-effort, never
            # fatal to the regen itself.
            try:
                replaced = set(self.tracked_invites.keys()) - set(new_invites.keys())
                if replaced:
                    guild_invites = await guild.invites()
                    for inv in guild_invites:
                        if inv.code in replaced:
                            try:
                                await inv.delete(reason="Invite batch auto-regenerated")
                            except discord.NotFound:
                                pass
                            except (discord.Forbidden, discord.HTTPException) as e:
                                logging.warning(f"[InviteManager] Could not delete replaced invite {inv.code}: {e}")
            except Exception as e:
                logging.warning(f"[InviteManager] Could not clean up replaced invites: {e}")
            for code in self.tracked_invites:
                self.tracked_invites[code]['expired'] = True
                self.tracked_invites[code]['status'] = 'expired'
            for code, data in new_invites.items():
                self.tracked_invites[code] = data
            pruned = self._prune_tracked()
            if pruned:
                logging.info(f"[InviteManager] Pruned {pruned} old expired entry(ies) from the tracked set (cap {_TRACKED_INVITE_CAP})")
            self.save_data()
            await self.update_invite_message(channel)
            return True
        return False
...
def register(bot: commands.Bot) -> None:
    ...
    async def regenerateinvites_cmd(ctx: commands.Context) -> None:
        ...
        async def progress(text: Optional[str] = None, color: Optional[discord.Color] = None,
                           embed: Optional[discord.Embed] = None) -> None:
            """Best-effort progress update — NEVER raises.

            The old implementation awaited progress_msg.edit(...) at three
            points with no error handling: if that one message got deleted
            (or the bot lost permission in the channel) mid-run, the whole
            command died and the embed stayed frozen on "Deleting old invite
            links..." forever. If the edit fails we fall back to sending a
            fresh progress message; even that failing must never abort the
            regeneration itself.
            """
            nonlocal progress_msg
            target = embed if embed is not None else progress_embed
            if text is not None:
                target.description = text
            if color is not None:
                target.color = color
            try:
                await progress_msg.edit(embed=target)
            except (discord.NotFound, discord.Forbidden, discord.HTTPException):
                try:
                    progress_msg = await ctx.send(embed=target)
                except discord.HTTPException:
                    pass

        # The whole regeneration is wrapped so ANY unexpected failure (a
        # permission revoked mid-run, a Discord 5xx, an edit storm hitting a
        # rate limit, ...) ends with a clear error state on the progress embed
        # instead of a frozen "Regenerating..." message with no explanation.
        try:
            await _run_regeneration(ctx, channel, progress)
        except asyncio.CancelledError:
            raise
        except Exception as e:
            logging.exception(f"[InviteManager] !regenerateinvites by {ctx.author} failed")
            await progress(f"❌ Regeneration failed: {e}", discord.Color.red())

    async def _run_regeneration(ctx: commands.Context, channel, progress) -> None:
        """Body of !regenerateinvites, kept as a helper so the command's
        error guard stays a clean wrapper. `progress` is the command's
        best-effort progress updater."""

        deleted_count = 0
        skipped_count = 0
        bot_user_id = ctx.bot.user.id if ctx.bot.user else None

        try:
            guild_invites = await ctx.guild.invites()
        except discord.Forbidden:
            await progress("❌ I don't have permission to manage invites.", discord.Color.red())
            return
        except discord.HTTPException as e:
            await progress(f"❌ Could not fetch this server's invites: {e}", discord.Color.red())
            return
        ...
        for invite in guild_invites:
            ...
            if is_bot_invite or is_tracked:
                to_delete.append(invite)
            else:
                skipped_count += 1
        total_to_delete = len(to_delete)

        async def _delete_one(invite) -> bool:
            """Delete one invite; True when it's gone from Discord."""
            try:
        ...
        # Delete in small concurrent chunks (bounded, so discord.py's internal
        # rate-limit handling still applies per request): ~5x faster than
        # one-at-a-time when the bot has a large accumulated backlog. Without
        # the live counter below, a multi-hundred-invite backlog looked
        # exactly like a hang — the embed sat on "Deleting old invite
        # links..." for minutes while the loop ground through hundreds of
        # rate-limited deletes in total silence (no log lines either, since
        # successful deletes never logged).
        CHUNK = 5
        last_edit = time.monotonic()
        for i in range(0, total_to_delete, CHUNK):
            chunk = to_delete[i:i + CHUNK]
            results = await asyncio.gather(*(_delete_one(inv) for inv in chunk))
            deleted_count += sum(1 for ok in results if ok)
            done = min(i + CHUNK, total_to_delete)
            now = time.monotonic()
            if done < total_to_delete and now - last_edit >= 3.0:
                await progress(f"⏳ Deleting old invite links... ({done}/{total_to_delete})")
                last_edit = now
        if total_to_delete:
            await progress(f"⏳ Deleted {deleted_count}/{total_to_delete} old invite link(s).")

        # Update progress: Generating new invites
        await progress("⏳ Generating new invite links...")
        ...
        if not new_invites:
            await progress("❌ Failed to create new invites. Make sure I have the `Create Invite` permission.", discord.Color.red())
            return

        # Update progress: Updating tracking message
        await progress("⏳ Updating tracking message...")
        ...
        await progress(embed=EmbedBuilder.success("✅ Invites Regenerated", summary))
