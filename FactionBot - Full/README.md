...
slim 299-line entry point (`src/bot.py`) plus layered infrastructure
...
├── src/
│   ├── bot.py            299-line entry: intents, TicketBot, registration
...
**Command surface** — 159 top-level / 191 qualified prefix commands, 0
...
slash machinery is gone) and `!botbranding name` (added as a parity port
from the Short variant). Everything else is byte-identical in names,
