...
Anchored to the variant root via ``core.paths`` so a root-level ``.env`` /
``tokens.txt`` is found from ANY launch directory.
"""
...
def load_local_env_file(env_path: Optional[str] = None) -> None:
    ...
    Search order (first existing file wins; later files are NOT merged in):
      ...
      2. ``.env`` at the variant root (``ShortVersion/.env``).
      3. ``.env`` inside ``src/`` (``ShortVersion/src/.env``).
